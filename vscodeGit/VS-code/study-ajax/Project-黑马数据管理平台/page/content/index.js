...
async function listHtml(){
    ...
    totalCount=res.data.total_count//3.1显示分页条数
...
document.querySelectorAll('.form-check-input').forEach(check=>{
    check.addEventListener('change',e=>{
        queryData.status=e.target.value
    })
})
document.querySelector('.form-select').addEventListener('change',e=>{
    queryData.channel_id=e.target.value
})
...
document.querySelector('.next').addEventListener('click',e=>{
    if(queryData.page<Math.ceil(totalCount/queryData.per_page)){
        queryData.page++
        document.querySelector('.page-now').innerHTML=`第${queryData.page}页`
        listHtml()
    }
})
...
/**
 * 目标4：删除功能
 *  4.1 关联文章 id 到删除图标，在便利时插入一个自定义属性id
 *  4.2 点击删除时，获取文章 id
...
document.querySelector('.art-list').addEventListener('click',async e=>{
    ...
    if(e.target.classList.contains('del')){
        ...
        //删除最后一页的最后一条时，需要自动向前翻页
        const children=document.querySelector('.art-list').children
        if(children.length===1&&queryData.page!==1){
            queryData.page--
            document.querySelector('.page-now').innerHTML=`第${queryData.page}页`
        }
        listHtml()
...
document.querySelector('.art-list').addEventListener('click',async e=>{
    ...
    if(e.target.classList.contains('edit')){ 
        const fromId=e.target.parentNode.dataset.id
        // console.log(fromId);
        location.href=`../publish/index.html?id=${fromId}`
    }
})
